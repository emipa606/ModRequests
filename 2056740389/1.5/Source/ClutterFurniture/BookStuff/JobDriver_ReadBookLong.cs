    ...
    public class JobDriver_ReadBookLong : JobDriver
    {
        ...
        public Toil ReadingBook(Pawn reader, int durationMultiplier, TargetIndex Ind, TargetIndex bookInd)
        {
            ...
            toil.tickAction = delegate
            {
                ...
                if (text.Count > 0)
                {
                    ...
                    {
                        s++;
                        MoteMaker.ThrowText(reader.TrueCenter() + new Vector3(0f, 0f, 0.7f),this.Map, text.ElementAt(s), Color.green);
                        reader.needs.joy.CurLevel += 0.03f;
                        i = 0;
                    }
                    i++;
                }
                //reader.Drawer.rotator.Face(thingBook.TrueCenter());
             };
        ...
        public Toil CarryBookToSeat(Pawn pawn)
        {
            ...
            carryBook.initAction = delegate
            {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        for (int i = 0; i < 4; i++)
                                        {
                                            Building edifice = (t.Position + GenAdj.CardinalDirections[i]).GetEdifice(this.Map);
                ...
                Thing thing = GenClosest.ClosestThingReachable(carryBook.actor.Position,this.Map, ThingRequest.ForGroup(ThingRequestGroup.BuildingArtificial), PathEndMode.OnCell, TraverseParms.For(carryBook.actor), 25f, validator, null, 1);
                if (thing != null)
                {
                    position = thing.Position;
                    this.Map.reservationManager.Reserve(carryBook.actor, thing);
                }
                else
                {
                    position = RCellFinder.SpotToChewStandingNear(carryBook.actor, carryBook.actor.CurJob.targetA.Thing);
                }
                this.Map.pawnDestinationManager.ReserveDestinationFor(carryBook.actor, position);
        ...
        public Toil PlaceItemToRead(TargetIndex bookInd)
        {
            ...
            placeItem.initAction = delegate
            {
                ...
                ClutterThingDefsFurniture clutterThingDefs = (ClutterThingDefsFurniture)carriedThing.def;
        ...
        public Toil TakeBookFromBookshelf(TargetIndex ind, Pawn reader, TargetIndex bookInd)
        {
            ...
            takeBook.initAction = delegate
            {
                ...
                if (this.Map.reservationManager.ReservedBy(bookshelf, reader))
                {
                    this.Map.reservationManager.Release(bookshelf, reader);
                }
        ...
        public bool TryFindAdjacentReadPlaceSpot(IntVec3 root, ThingDef bookDef, out IntVec3 placeSpot)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                bool arg_75_0;
                if (intVec.HasEatSurface(this.Map))
                {
                    arg_75_0 = (
                        from t in this.Map.thingGrid.ThingsAt(intVec)
                        where t.def == bookDef
                        select t).Any<Thing>();
                }
                else
            ...
            if (!placeSpot.IsValid)
            {
                ...
                for (int j = 0; j < list.Count; j++)
                {
                    ...
                    bool arg_125_0;
                    if (intVec2.Walkable(this.Map))
                    {
                        arg_125_0 = (
                            from t in this.Map.thingGrid.ThingsAt(intVec2)
                            where t.def == bookDef
                            select t).Any<Thing>();
                    }
                    else
