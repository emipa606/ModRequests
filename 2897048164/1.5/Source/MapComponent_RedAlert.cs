    ...
    public class MapComponent_RedAlert : MapComponent
    {
            ...
            if (map.IsPlayerHome && ResearchProjectDefOf.ColoredLights.IsFinished)
            {
                ...
                if (RedAlertMod.settings.autoRestrictUndraftedPawnsToHomeArea)
                {
                    if (threats.Any())
                    {
                        ...
                        {
                            if (colonist.playerSettings.AreaRestrictionInPawnCurrentMap != map.areaManager.Home)
                            {
                                pawnsWithOldAreas[colonist] = colonist.playerSettings.AreaRestrictionInPawnCurrentMap;
                                colonist.playerSettings.AreaRestrictionInPawnCurrentMap = map.areaManager.Home;
                            }
                        }
                    }
                    else
                    {
                        ...
                        foreach (var kvp in pawnsWithOldAreas)
                        {
                            ...
                            {
                                kvp.Key.playerSettings.AreaRestrictionInPawnCurrentMap = kvp.Value;
                                keys.Add(kvp.Key);
                            }
                        }
        ...
        private void EnableLamp(ColorInt threatColor, ThingWithComps lamp)
        {
            ...
            if (comp != null)
            {
                ...
                map.mapDrawer.MapMeshDirty(comp.parent.Position, MapMeshFlagDefOf.Things);
            ...
            else
            {
                ...
                if (compOffset != null)
                {
                    ...
                    map.mapDrawer.MapMeshDirty(compOffset.parent.Position, MapMeshFlagDefOf.Things);
                ...
                else
                {
                    ...
                    if (compGlowerExtended != null)
                    {
                        ...
                        map.mapDrawer.MapMeshDirty(compGlowerExtended.parent.Position, MapMeshFlagDefOf.Things);
                        map.glowGrid.RegisterGlower(compGlower);
