...
namespace StatueOfAnimal {
    ...
    public class Dialog_PresetList : Window {
        ...
        private List<StatueOfAnimalPreset> presets = new List<StatueOfAnimalPreset>();
        ...
        public Dialog_PresetList(Building_StatueOfAnimal statue, Window_EditStatue parent) {
        ...
        public override void DoWindowContents(Rect inRect) {
            ...
            float num2 = 0f;
            int num3 = 0;
            foreach (StatueOfAnimalPreset current in this.presets) {
                ...
                if (Widgets.ButtonText(rect4, "StatueOfAnimal.LoadPreset".Translate(), true, false, current.IsValid)) {
                    Load(current);
                }

                {
                    ...
                    TooltipHandler.TipRegion(rectTextField, "StatueOfAnimal.WeightInputFormDesc".Translate());
                ...
                TooltipHandler.TipRegion(rect5, "StatueOfAnimal.DeleteThisPreset".Translate());
                GUI.EndGroup();
                num2 += vector.y + 3f;
                num3++;
            }
        ...
        public void Load(StatueOfAnimalPreset preset) {
            this.statue.CopyStatueOfAnimalFromPreset(preset);
            this.parent.UpdateButtonLabel();
            this.Close(true);
        }

        public void Save(StatueOfAnimalPreset preset) {
            this.presets.Add(preset);
            SaveSettings();
        }

        public void Remove(StatueOfAnimalPreset preset) {
            this.presets.Remove(preset);
            SaveSettings();
        }

        public void LoadSettings() {
            this.presets = StatueOfAnimalPref.presets;
            if (this.presets == null) {
                Log.Message("StatueOfAnimalPreset is null");
                this.presets = new List<StatueOfAnimalPreset>();
            }
        }

        public void SaveSettings() {
            StatueOfAnimalPref.presets = this.presets;
            StatueOfAnimalPref.SavePref();
        }
        ...
        public void DoTypeInField(Rect rect) {
            ...
            if (Widgets.ButtonText(rect3, "StatueOfAnimal.SavePresetButton".Translate(), true, false, true) || flag) {
                if (this.typingName.NullOrEmpty()) {
                    Messages.Message("NeedAName".Translate(), MessageTypeDefOf.RejectInput);
                } else {
                    this.Save(new StatueOfAnimalPreset(this.typingName, statue));
                }
            }
            Text.Anchor = TextAnchor.UpperLeft;
            GUI.EndGroup();
        ...
        public Color PresetNameColor(StatueOfAnimalPreset preset) {
