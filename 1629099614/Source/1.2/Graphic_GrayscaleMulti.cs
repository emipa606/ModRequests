...
namespace StatueOfAnimal {
    public class Graphic_GrayscaleMulti : Graphic {
        ...
        public override void Init(GraphicRequest req) {
            Log.Message("[Grayscale Multi Init]" + req.path + "/" + req.color +"/" + req.shader.name);
            ...
            if (array[0] == null) {
                if (array[2] != null) {
                    ...
                    if (!(array[3] != null)) {
                        Log.Error("Failed to find any texture while constructing " + this.ToStringSafe<Graphic_GrayscaleMulti>() + ". Filenames have changed; if you are converting an old mod, recommend renaming textures from *_back to *_north, *_side to *_east, and *_front to *_south.", false);
                        return;
                    }
                    array[0] = array[3];
                    this.drawRotatedExtraAngleOffset = 90f;
