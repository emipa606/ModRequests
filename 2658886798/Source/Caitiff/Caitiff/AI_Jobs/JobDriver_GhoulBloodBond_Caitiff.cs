...
namespace Vampire
{
    public class JobDriver_GhoulBloodBond_Caitiff : JobDriver
    {
            ...
            Toil grappleToil = new Toil()
            {
                initAction = delegate
                {
                    ...
                    if (victim != null)
                    {
                        ...
                        if (newDomitor.HasVampireHediffs())
...
//                            }
							...
							int xp = 0;
							if (CaitiffFunctions.IsBloodline(newDomitor, VampDefOf.ROMV_ClanCaitiff))
								xp = Improved_VampireSettings.Get.CaitiffXP;

							//if (CaitiffFunctions.IsBloodline(newDomitor, VampDefOf.ROMV_ClanCaitiff))  //IsBloodline(actor, VampDefOf.ROM_VampirismCaitiff)) //actor?.health?.hediffSet?.HasHediff(VampDefOf.ROM_VampirismCaitiff) ?? false)
							//{
							//	xp = xp / 3;
							//}
							...
							if (newDomitor?.VampComp() is CompVampire v && v.IsVampire && newDomitor.Faction == Faction.OfPlayer)
                                {
                                    MoteMaker.ThrowText(newDomitor.DrawPos, newDomitor.Map, "XP +" + xp);
                                    v.XP += xp;
                                }
                                workLeft = BaseFeedTime;
                                MoteMaker.MakeColonistActionOverlay(newDomitor, ThingDefOf.Mote_ColonistAttacking);
