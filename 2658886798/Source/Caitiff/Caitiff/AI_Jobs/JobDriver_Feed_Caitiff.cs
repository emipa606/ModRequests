...
namespace Vampire
{
	public class JobDriver_Feed_Caitiff : JobDriver
	{
		private float workLeft = -1f;
		public static float BaseFeedTime = 320f;
		public static float BaseCoolantThrowupChance = 0.25f;
		//public bool IsCaitiff => Pawn.health.hediffSet.HasHediff(VampDefOf.ROM_VampirismCaitiff);
					...
					if (TargetA.Thing is Pawn victim && victim.Spawned && !victim.Dead)
					{
						...
						if (workLeft <= 0f)
						{
							...
							if (!stopCondition(actor, victim))
							{
								...
								if (/*actor.IsVampire()*/ actor.HasVampireHediffs() && cleansWound) VampireBiteUtility.CleanBite(actor, victim);
							}
							else
							{
