...
namespace BDsPlasmaWeaponVanilla
{
    ...
    public class CompReloadableFromFiller : CompEquippedGizmo, IVerbOwner, ICompWithCharges
    {
        public int RemainingCharges;

        private VerbTracker verbTracker;

        public CompProperties_ApparelReloadable Props => props as CompProperties_ApparelReloadable;
        ...
        int ICompWithCharges.RemainingCharges => RemainingCharges;
        ...
        public void DrawGas(int amount, bool doFleck)
        {
            ...
            if (EquipmentSource != null)
            {
                map = EquipmentSource.PrimaryVerb.Caster.Map;
                pos = EquipmentSource.PrimaryVerb.Caster.Position;
            }
            ...
            else
            {
                ...
                if (doFleck)
                {
                    for (int i = 0; i < amount; i++)
                    {
                        Effecter effecter = BDStatDefOf.LizionCoolerHigh.Spawn(parent.Position, map);
                        effecter.Trigger(target, target);
                    }
                }
            }
    ...
    public class JobGiver_ReloadFromFiller : ThinkNode_JobGiver
    {
        ...
        public static Thing FindFiller(Pawn pawn)
        {
            ...
            {
                if (x.def != ThingDefOf.BDP_LizionFillerLarge)
                {
                    return false;
                }
