        ...
        protected override Job TryGiveJob(Pawn pawn)
        {
            if (pawn.Faction == Faction.OfPlayer && !pawn.WorkTagIsDisabled(WorkTags.Violent) && pawn.Drafted && pawn.drafter.FireAtWill && !pawn.WorkTagIsDisabled(WorkTags.Firefighting))
            {
                ...
                if (weapon != null && weapon.def.weaponTags != null &&
                    (!weapon.def.weaponTags.NullOrEmpty() && weapon.def.weaponTags.Contains("BDP_FireExtinguisher") || (weapon.def.weaponTags.Contains("BDP_FireExtinguisherSecondary") && weapon.TryGetComp<CompSecondaryVerbCE>().IsSecondaryVerbSelected))
