...
//				var postfix = new HarmonyMethod(typeof(TakeMealFromDispenser_Patch), Utility.IsReplimat() ? nameof(TakeMealFromDispenser_Patch.PostfixReplimat) : nameof(TakeMealFromDispenser_Patch.Postfix));
				var postfix = new HarmonyMethod(typeof(TakeMealFromDispenser_Patch), SaferPasteUtility.IsIncompatibleMod() ? "PostfixIncompatible" : "Postfix");
			...
			static void PostfixIncompatible(Toil __result, TargetIndex ind, Pawn eater)
			{
				if (eater.jobs.curJob.GetTarget(ind).Thing.IsIncompatibleMod())
				{
					return;
				}
				__result.initAction = delegate
