...
namespace SaferPasteDispenser
{
	static class ExtensionMethod
	{
		public static Thing TryDispenseSaferFood(this Building_NutrientPasteDispenser dispenser, Pawn actor)
		{
			...
			if (Settings.numberOfIngredients != Settings.NumberOfIngredients.Single)
			{
				...
				{
					var feed = feedGroup.feed;
					var feedNutrition = feed.GetStatValue(StatDefOf.Nutrition);
			...
			IEnumerable<Thing> FindFeedsAmongAllHoppers()
			{
				...
				foreach (var AdjCells in dispenser.AdjCellsCardinalInBounds)
				{
					...
					{
						if (adjThing.def == ThingDefOf.Hopper)
						{
							isHopper = true;
						}
		...
		internal static IEnumerable<Thing> FindFeedsAmongAllHoppers(this Building_NutrientPasteDispenser dispenser)
		{
			...
			foreach (var AdjCells in dispenser.AdjCellsCardinalInBounds)
			{
				...
				foreach (var adjThing in AdjCells.GetThingList(dispenser.Map))
				{
					if (adjThing.def == ThingDefOf.Hopper || adjThing.IsRimFridge() || adjThing.IsNPDTiers())
					{
						isHopper = true;
					}
					else if (Building_NutrientPasteDispenser.IsAcceptableFeedstock(adjThing.def))
	...
	static class ExtensionMethodRimFridge
	{
		private static Type cacheRimFridge_Building = null;
		private static bool hasChachedRimFridge_Building = false;

		internal static bool IsRimFridge(this Thing hopper)
		{
			if (!SaferPasteUtility.IsRimFridge())
			{
				return false;
			}
			if (!hasChachedRimFridge_Building)
			{
				cacheRimFridge_Building = AccessTools.TypeByName("RimFridge.RimFridge_Building");
				hasChachedRimFridge_Building = true;
			}
			return hopper.GetType() == cacheRimFridge_Building;
		}
	}
}
