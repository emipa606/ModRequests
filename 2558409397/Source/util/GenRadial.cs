...
namespace YayoNature
{
	public static class GenRadial
	{
		...
		static int setupTryCount = 0;
		static bool isSetuped = false;
		...
		static public void setupAuto()
		{
			while (!isSetuped && setupTryCount < 5)
			{
				try
				{
					Log.Message("GenRadial try setup");
					setupTryCount++;
					setup();
				}
				catch
		...
		static public void setup()
        {
			...
			for (int i = 0; i < ar_NumCellsInRadius_rcos.Length; i++)
			{
			...
			Log.Message("GenRadial Generated");
		}
		...
		private static void SetupRadialPattern()
		{
			...
			for (int k = 0; k < RadialPatternCount; k++)
			{
