...
namespace Metro
{
	public class JobGiver_GetFood : ThinkNode_JobGiver
	{
		...
		public override Job TryGiveJob(Pawn pawn)
		{
			...
			var cocoons = new List<Cocoon>();
			foreach (var cell in cells)
            {
				foreach (var t in cell.GetThingList(pawn.Map))
                {
					if (t is Cocoon cocoon && cocoon.ContainedThing != null)
                    {
						cocoons.Add(cocoon);
                    }
                }
			...
			if (cocoons.Count > 0)
            {
				var chosenCocoon = cocoons.RandomElement();
				Job job4 = JobMaker.MakeJob(MetroDefOf.Metro_IngestFromCocoon, chosenCocoon.ContainedThing, chosenCocoon);
				job4.count = 1;
				return job4;
			}
