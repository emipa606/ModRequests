...
namespace Metro
{
	public class JobGiver_GetRest : ThinkNode_JobGiver
	{
		...
		private IntVec3 FindGroundSleepSpotFor(Pawn pawn)
		{
			...
			var cellsCandidates = cells.Where(x => x.Walkable(map) && pawn.CanReserve(x));
			if (cellsCandidates.Count() > 0)
            {
				return cellsCandidates.RandomElement();
            }
			for (int i = 0; i < 2; i++)
			{
				int radius = (i == 0) ? 4 : 12;
				if (CellFinder.TryRandomClosewalkCellNear(pawn.Position, map, radius, out IntVec3 result, 
