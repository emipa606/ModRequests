...
namespace ScenarioPawnsAndCorpses
{
    ...
    static class HarmonyPatches
    {
        ...
        private static RelationshipOverrides GetRelationshipOverrides(Pawn pawn, BackstorySlot slot)
        {
            ...
            {
                relationshopOverrides = DefDatabase<BackstoryDef>.GetNamedSilentFail(backstoryIdString)?.relationshipOverrides;
            }
        ...
        public static IEnumerable<CodeInstruction> TryGenerateNewPawnInternal_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            // 		IL_00e3: call instance bool Verse.PawnGenerationRequest::get_Newborn()
            ...
            {
                if (!ModsConfig.IsActive("erdelf.HumanoidAlienRaces") && !newbornCheckSkipped && i + 1 < instructionCount && instructions.ElementAt(i + 1).OperandIs(newbornInfo))
                {
