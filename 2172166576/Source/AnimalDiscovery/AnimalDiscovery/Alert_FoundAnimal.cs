...
namespace AnimalDiscovery
{
	public class Alert_FoundAnimal : Alert
	{
			...
			get
			{
				...
				for (int i = 0; i < maps.Count; i++)
				{
					...
					if (AnimalDiscoveryMod.Settings.customAlert && (map.IsPlayerHome || AnimalDiscoveryMod.Settings.notOnlyPlayerHome) && map.mapPawns.AnyColonistSpawned)
					{
						...
						{
							Pawn pawn = t as Pawn;
							return pawn != null && !pawn.Fogged() && AnimalDiscoveryMod.Settings.IsAlertTarget(pawn);
						};
						this.cacheFoundWildAnimals.AddRange(from t in map.listerThings.ThingsInGroup(ThingRequestGroup.Pawn)
															 where validator(t)
															 select t);
					}
				}
		...
		private List<Thing> cacheFoundWildAnimals = new List<Thing>();
	}
}