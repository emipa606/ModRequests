    ...
    public class AnimalDiscoveryMod : Mod
    {
		...
		public override void DoSettingsWindowContents(Rect inRect)
		{
			...
			if (Settings.alertTargetAnimal == AnimalDiscoverySettings.AlertTargetAnimal.Custom)
			{
				...
				Rect settingArea = new Rect(0f, num - 5f, 250f, 0f);
                
				// リストの並び順
				//Widgets.Label(new Rect(0f, num, 300f, 40f), "AnimalDiscovery.SortList".Translate());
				...
				// プリセットをロード プリセットを作成
				float columwidth = (inRect.width - 40f) / 2f;
		...
		private void DrawAddAnimalDefButton(Rect rect)
		{
			...
			{
				List<ThingDef> list = new List<ThingDef>();
				list.AddRange(DefDatabase<ThingDef>.AllDefsListForReading.Where(x => !settings.settingItems.Exists((AnimalAlertSettingItem item) => item.def == x) && x.race != null && x.race.Animal));
				if (!list.NullOrEmpty<ThingDef>())
				{
