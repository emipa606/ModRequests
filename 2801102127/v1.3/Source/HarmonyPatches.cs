				...
				*/
				if (condition == JobCondition.InterruptOptional || condition == JobCondition.InterruptForced)
				{
					...
					if (job?.def is JobDef jobDef)
					{
						...
						if (driver is JobDriver_DoBill driver_DoBill)
						{
...
//											ingredients.Add(job.targetB.Thing.def);
									...
									{
										float xp = driver_DoBill.ticksSpentDoingRecipeWork * 0.1f * recipe.workSkillLearnFactor;
...
//						else if (driver is JobDriver_PlantCut || driver is JobDriver_PlantCut_Designated)
						else if (driver is JobDriver_PlantWork)
						{
							var plant = job.targetA.Thing;
							var workDone = Traverse.Create(driver).Field<float>("workDone").Value;
							if (workDone > 0)
							{
								...
								ResumeDictionary.Instance.SetResumeWorkDone(plant, jobDef, Mathf.Min(workDone, plant.def.plant.harvestWork), efficiency);
							}
						}
						...
						{
							if (pawn.jobs.curJob.GetTarget(TargetIndex.B).Thing is Blueprint blueprint)
							{
						...
						else if (driver is JobDriver_Deconstruct && !Settings.IsDisabledDeconstruct
							|| driver is JobDriver_Uninstall && !Settings.IsDisabledUninstall
...
//							var workLeft = AccessTools.FieldRefAccess<float>(driver.GetType(), "workLeft")(driver);
		...
		public class Notify_DoBillStarted_Patch
		{
			[HarmonyPostfix]
...
//			static void Postfix(JobDriver __instance, ref List<Toil> ___toils, Pawn ___pawn, Job ___job)
			static void Postfix(JobDriver __instance, ref List<Toil> ___toils)
			{
				...
				{
					if (__instance is JobDriver_Deconstruct && Settings.IsDisabledDeconstruct)
					{
						return;
					}
					if (__instance is JobDriver_Uninstall && Settings.IsDisabledUninstall)
					{
						return;
					}
					___toils[1].initAction = () =>
...
//				else if (__instance is JobDriver_PlantCut || __instance is JobDriver_PlantCut_Designated)
				else if (__instance is JobDriver_PlantWork)
				{
...
//						workDone = ResumeDictionary.Instance.GetResumeWorkDone(plant, job.def, out _);
						if (!ResumeDictionary.Instance.GetResumeWorkDone(plant, job.def, out float workDone, out _))
						{
							workDone = 0f;
						}
						var traverse = Traverse.Create(__instance).Field("workDone").SetValue(workDone);
					};
					___toils[5].tickAction = () =>
					{
						...
						{
							actor.skills.Learn(SkillDefOf.Plants, Traverse.Create(__instance).Field<float>("xpPerTick").Value);
						}
						Job job = __instance.job;
						Plant plant = (Plant)job.targetA.Thing;
						...
						workDone += JobDriver_PlantWork.WorkDonePerTick(actor, plant);
						if (workDone >= property.harvestWork)
						{
							if (property.harvestedThingDef is not null)
							{
								...
								ResumeDictionary.Instance.SetResumeWorkDone(plant, job.def, Mathf.Min(workDone, property.harvestWork), efficiency);
				...
				else if (__instance is JobDriver_Replant)
				{
					...
					{
						var blueprint = __instance.job.GetTarget(TargetIndex.B).Thing as Blueprint;
//						ref var workDone = ref AccessTools.FieldRefAccess<float>(__instance.GetType(), "workDone")(__instance);
//						workDone = ResumeDictionary.Instance.GetResumeWorkDone(plant, job.def, out _);
