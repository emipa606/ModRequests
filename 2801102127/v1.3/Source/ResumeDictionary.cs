...
namespace ResumableJobProgress
{
	class ResumeDictionary
	{
		...
		class ResumeJob : IExposable
		{
			...
			public bool HasTempThing => !(targetThingDefTemp is null);
		...
		private Dictionary<(TargetInfo target, Def def), (float, float)> resumeJobWorkLefts = new Dictionary<(TargetInfo, Def), (float, float)> { };
				...
				try
				{
					if (Scribe.mode == LoadSaveMode.Saving)
					{
						...
						foreach (var resumeJobWorkLeft in resumeJobWorkLefts)
						{
							...
							if (target.IsValid && !(target.Map is null))
							{
