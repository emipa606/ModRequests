...
namespace VTESoS
{
    public class VTESoS2Mod : Mod
    {
        ...
        public VTESoS2Mod(ModContentPack content) : base(content)
        {
            ...
            {
                Harm.Patch(AccessTools.Method(typeof(ShipInteriorMod2), nameof(ShipInteriorMod2.DefsLoaded)),
                    new HarmonyMethod(GetType(), nameof(DefsLoaded_Prefix)), new HarmonyMethod(GetType(), nameof(DefsLoaded_Postfix)));
            });
        }

        public static void DefsLoaded_Prefix(ref bool ___loadedGraphics, out bool __state)
        {
            __state = ___loadedGraphics;
            ___loadedGraphics = true;
        }

        public static void DefsLoaded_Postfix(bool __state, ref bool ___loadedGraphics)
        {
            if (__state) return;
