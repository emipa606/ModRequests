...
namespace ARoM_SpaceTimeMage
{
    ...
    public class Verb_TimeJaunt : Verb_UseAbility
    {
        int pwrVal = 0;
        float arcaneDmg = 1;
        Pawn pawn;
        ...
        protected override bool TryCastShot()
        {
            ...
            if (map != null)
            {
                ...
                TorannMagic.CompAbilityUserMagic comp = pawn.GetComp<TorannMagic.CompAbilityUserMagic>();
                //pwrVal = TM_Calc.GetMagicSkillLevel(this.pawn, comp.MagicData.MagicPowerSkill_LightSkip, "TM_LightSkip", "_pwr", false);
                pwrVal = TorannMagic.TM_Calc.GetSkillPowerLevel(pawn, TorannMagic.TorannMagicDefOf.TM_LightSkip, false);
                ...
                List<Pawn> tmpList = TorannMagic.TM_Calc.FindAllPawnsAround(this.CasterPawn.Map, launcherPosition, 1f, this.pawn.Faction, true);
                for (int i = 0; i < tmpList.Count; i++)
                {
                    if (TorannMagic.ModCheck.Validate.GiddyUp.Core_IsInitialized())
                    {
                        if (TorannMagic.ModCheck.GiddyUp.IsMount(tmpList[i]))
                        {
                            continue;
                        }
                    }
            ...
            {
                MoteMaker.ThrowText(pawn.DrawPos, pawn.Map, "Time Jaunt: " + StringsToTranslate.AU_CastFailure, -1f);
            }
            return result;
        }
        ...
        public void CreatePodGroup()
        {
            if (pawnList != null && pawnList.Count > 0)
            {
                for (int i = 0; i < pawnList.Count; i++)
                {
                    ...
                    if (TorannMagic.ModCheck.Validate.GiddyUp.Core_IsInitialized())
                    {
                        mount = TorannMagic.ModCheck.GiddyUp.GetMount(p);
                        TorannMagic.ModCheck.GiddyUp.ForceDismount(p);
                    }
                    Thing pod = ThingMaker.MakeThing(TorannMagic.TorannMagicDefOf.TM_LightPod, null);
        ...
        public void LaunchLightPod(int destinationTile, IntVec3 destinationCell, TransportPodsArrivalAction arrivalAction)
        {
            ...
            for (int i = 0; i < podTList.Count; i++)
            {
                ...
                TorannMagic.WorldTransport.TM_DropPodLeaving obj = (TorannMagic.WorldTransport.TM_DropPodLeaving)SkyfallerMaker.MakeSkyfaller(TorannMagic.TorannMagicDefOf.TM_LightPodLeaving, activeDropPod);
                obj.groupID = groupID;
            ...
            if (!map.mapPawns.AnyColonistSpawned && !map.IsPlayerHome)
            {
                ...
                Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(stringBuilder.ToString(), delegate
                {
                    ...
                    TorannMagic.WorldTransport.TM_DelayedDestroyMap ddm = new TorannMagic.WorldTransport.TM_DelayedDestroyMap();
                    ddm.parent = sm;
                    ddm.delayTicks = 120;
                    sm.AllComps.Add(ddm);
                }));
        ...
        private void StartChoosingDestination()
        {
            ...
            Find.WorldTargeter.BeginTargeting(new Func<GlobalTargetInfo, bool>(this.ChooseWorldTarget), true, Verb_TimeJaunt.TargeterMouseAttachment, true, delegate
            {
