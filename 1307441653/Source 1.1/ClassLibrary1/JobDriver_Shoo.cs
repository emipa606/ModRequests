            ...
            shoo.initAction = delegate
            {
                ...
                if (Rand.Chance(successOdds))
                {
                    ...
                    if (FindShooFleeCell(pawn, out IntVec3 c))
                    {
                        ...
                        pawn.jobs.TryTakeOrderedJob(new Job(ShooDefOf.FleeBecauseShoo, c, shoo.actor) { canBash = true }); // SOMEHOW canBash makes it work for herd animals. Don't ask me to explain it, because I can't.
