...
namespace Copyprint
{
    public class Designator_Copyprint : Designator
    {
        #region Fields
        ...
        public Designator_Copyprint( Copyprint copyprint )
        {
            _copyprint = copyprint;
            icon = Resources.Icon_Copyprint;
        ...
        public Copyprint Copyprint => _copyprint;
        public override int DraggableDimensions => 0;
        public override string Label => _copyprint.name;
        public override IEnumerable<FloatMenuOption> RightClickFloatMenuOptions
        {
            get
            {
                ...
                {
                    // rename
                    new FloatMenuOption("Nuka.Copyprints.Rename".Translate(), delegate
                    { Find.WindowStack.Add(new Dialog_NameCopyprint(this.Copyprint)); }),

                    // delete copyprint
                    new FloatMenuOption("Nuka.Copyprints.Remove".Translate(), delegate
                    { Controller.Remove(this, false); })
                };

                // delete copyprint and remove from disk
                if (this.Copyprint.exported)
                {
                    options.Add(new FloatMenuOption("Nuka.Copyprints.RemoveAndDeleteXML".Translate(), delegate
                ...
                {
                    options.Add(new FloatMenuOption("Nuka.Copyprints.SaveToXML".Translate(), delegate
                    { Controller.SaveToXML(this.Copyprint); }));
                }
        ...
        // note that even though we're designating a copyprint, as far as the game is concerned we're only designating the _origin_ cell
        public override void DesignateSingleCell( IntVec3 origin )
        {
            bool somethingSucceeded = false;
            bool planningMode = Event.current.shift;
            var cells = Copyprint.AvailableContents.Select(cell => cell.Position);
            foreach (var cell in cells)
            {
                if ((cell + origin).InBounds(Map))
                {
                    List<Thing> destroyTargetList = (cell + origin).GetThingList(Map);
                    for (int index = 0; index < destroyTargetList.Count; index++)
                    {
                        if (destroyTargetList[index].def != null && destroyTargetList[index].def.destroyable && !(destroyTargetList[index] is Pawn))
                        {
                            destroyTargetList[index].Destroy(DestroyMode.Vanish);
                        }
                    }
                }
            ...
            foreach ( var item in Copyprint.AvailableContents )
            {
                if((item.Position + origin).InBounds(Map))
                {
                    if (item.BuildableDef is TerrainDef)
                    {
                        Map.terrainGrid.SetTerrain(item.Position + origin, (TerrainDef)item.BuildableDef);
                    }
                    else
                    {
                        Thing thing = ThingMaker.MakeThing((ThingDef)item.BuildableDef, item.Stuff);
                        ...
                        if (thing is Plant)
                        {
                            ((Plant)thing).Growth = item._growth;
                        }
                        if (item._count > 1)
                        {
                            thing.stackCount = item._count;
                        }
                        if (item._qualityCategory != QualityCategory.Normal)
                        {
                            thing.TryGetComp<CompQuality>().SetQuality(item._qualityCategory, ArtGenerationContext.Colony);
                        }
                        GenSpawn.Spawn(thing, item.Position + origin, Map, item._rotation, WipeMode.Vanish, false);
                    }
                }             
            }
        ...
        public override void DoExtraGuiControls( float leftX, float bottomY )
        {
            ...
            Find.WindowStack.ImmediateWindow( 73095, winRect, WindowLayer.GameUI, delegate
            {
                ...
                {
                    Copyprint.Rotate( rotationDirection );
                }
        ...
        public override void DrawPanelReadout( ref float curY, float width )
        {
            ...
            foreach ( var buildables in Copyprint.GroupedBuildables )
            {
                // count
                float curX = 5f;
                ...
                if ( buildables.Value.First().Stuff != null )
                {
                    ...
                    if ( Widgets.ButtonInvisible( buttonRect ) )
                        Copyprint.DrawStuffMenu( buildables.Key );
                }
                else
                {
                    // label
            ...
            curY += 12f;
            Text.Font = GameFont.Small;
            Widgets.Label( new Rect( 0f, curY, width, 24f ), "Nuka.Copyprints.Cost".Translate() );
            curY += 24f;

            Text.Font = GameFont.Tiny;
            List<ThingDefCount> costlist = Copyprint.CostListAdjusted;
        ...
        */
        ...
        {
            return Copyprint == ( other as Designator_Copyprint ).Copyprint;
        }

        public override void Selected()
        {
            base.Selected();
            Copyprint.RecacheBuildables();
            /*if ( !Copyprint.AvailableContents.Any() )
            {
                Messages.Message("Nuka.Copyprints.NothingAvailableInCopyprint".Translate( Copyprint.name ),
                                  MessageTypeDefOf.RejectInput );
            }*/
            /*else
            {
                var unavailable = Copyprint.contents.Except( Copyprint.AvailableContents ).Select( bi => bi.BuildableDef.label ).Distinct();
                if ( unavailable.Any() )
                    Messages.Message("Nuka.Copyprints.XNotAvailableInCopyprint".Translate( Copyprint.name, string.Join( ", ", unavailable.ToArray() ) ), MessageTypeDefOf.CautionInput );
            }*/
        }
        ...
        private void HandleRotationShortcuts()
        {
            ...
            {
                SoundDefOf.AmountIncrement.PlayOneShotOnCamera();
                Copyprint.Rotate( RotationDirection.Clockwise );
            }
            if ( rotationDirection == RotationDirection.Counterclockwise )
            {
                SoundDefOf.AmountDecrement.PlayOneShotOnCamera();
                Copyprint.Rotate( RotationDirection.Counterclockwise );
            }
        }
