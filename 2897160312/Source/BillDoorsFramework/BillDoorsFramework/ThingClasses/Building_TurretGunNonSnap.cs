...
{
    public class Building_TurretGunNonSnap : Building_TurretMulti, IFireArc
    {
    ...
    public class ModExt_NonSnapTurret : DefModExtension
    {
    ...
    public static class AttackTargetFinderAngle
    {
        ...
        public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Vector3 angle, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f)
        {
            ...
            {
                Thing thing = t.Thing;
                if (t == searcher) return false;
                if (minDistSquared > 0f && (searcherThing.Position - thing.Position).LengthHorizontalSquared < minDistSquared) return false;

                float num3 = verb.verbProps.EffectiveMinRange(thing, searcherThing);
                if (num3 > 0f && (searcherThing.Position - thing.Position).LengthHorizontalSquared < num3 * num3) return false;
                ...
                {
                    if (losValidator != null && (!losValidator(searcherThing.Position) || !losValidator(thing.Position))) return false;
                    if (!searcherThing.CanSee(thing, losValidator))
                    {
                        if (t is Pawn)
                        {
                            if ((flags & TargetScanFlags.NeedLOSToPawns) != 0) return false;
                        }
                        else if ((flags & TargetScanFlags.NeedLOSToNonPawns) != 0) return false;
                    }
                }
                if (((flags & TargetScanFlags.NeedThreat) != 0 || (flags & TargetScanFlags.NeedAutoTargetable) != 0) && t.ThreatDisabled(searcher)) return false;
                if ((flags & TargetScanFlags.NeedAutoTargetable) != 0 && !Verse.AI.AttackTargetFinder.IsAutoTargetable(t)) return false;
                if ((flags & TargetScanFlags.NeedActiveThreat) != 0 && !GenHostility.IsActiveThreatTo(t, searcher.Thing.Faction)) return false;
                Pawn pawn = t as Pawn;
                if (onlyTargetMachines && pawn != null && pawn.RaceProps.IsFlesh) return false;
                if ((flags & TargetScanFlags.NeedNonBurning) != 0 && thing.IsBurning()) return false;
                if (searcherThing.def.race != null && (int)searcherThing.def.race.intelligence >= 2)
                {
                    CompExplosive compExplosive = thing.TryGetComp<CompExplosive>();
                    if (compExplosive != null && compExplosive.wickStarted) return false;
                }
        ...
        {
            if (!(t is Pawn pawn)) return false;
            if (pawn.IsCombatant()) return false;
            if ((flags & TargetScanFlags.IgnoreNonCombatants) != 0) return true;
            if (GenSight.LineOfSightToThing(searcherThing.Position, pawn, searcherThing.Map)) return false;
            return true;
        }
        ...
        {
            if (GetAvailableShootingTargetsByScore(targets, searcher, verb, angle).TryRandomElementByWeight((Pair<IAttackTarget, float> x) => x.Second, out var result)) return result.First;
            return null;
        }
        ...
        private static float GetShootingTargetScore(IAttackTarget target, IAttackTargetSearcher searcher, Verb verb, Vector3 angle)
        {
            ...
            var ext = searcher.Thing.def.GetModExtension<ModExt_NonSnapTurret>();
            var anglef = Vector3.Angle(angle, (target.Thing.DrawPos - searcher.Thing.DrawPos).Yto0());
            if (ext == null)
            {
                if (anglef < 0.1f) anglef = 0.1f;
                return num / anglef;
            }
