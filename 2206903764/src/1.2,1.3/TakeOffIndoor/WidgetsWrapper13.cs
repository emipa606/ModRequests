...
namespace TakeOffIndoor
{
    public class ListingStandardWrapper
    {
        ...
        public void EndWithScroll()
        {
            ...
            {
                scrollViewHeight = curHeight + 30f;
            }
        ...
        public Rect ColRect(float leftMag = 0f, float widthMag = 1f,float height=-1f)
        {
            if (height == -1f) height = DefaultGap;
            top = curHeight;    bottom = curHeight + height;
            return OneColumnRect(curHeight, curHeight + DefaultGap, leftMag, widthMag);
        }
        public Rect ColRectGap(float leftMag = 0f, float widthMag = 1f, float height = -1f)
        {
            if (height == -1f) height = DefaultGap;
            Rect tmp = OneColumnRect(curHeight, curHeight + height, leftMag, widthMag);
            top = curHeight;    curHeight += height;    bottom = curHeight;
            return tmp;
        }
        public Rect PrevColRect(float leftMag = 0f, float widthMag = 1f)
        {
            return OneColumnRect(top, bottom, leftMag, widthMag);
        }
        public Rect OneColumnRect(float top, float bottom, float leftMag = 0f, float widthMag = 1f)
        {
            return new Rect((float)(viewRect.x + viewRect.width * leftMag), top, (float)(viewRect.width * widthMag), bottom - top);
        }
        ...
        public void LabelSized(string leftString, string tip = null, float leftMag = 0f, float widthMag = 1f)
        {
            Tooltip(ColRect(leftMag, widthMag), tip);
            Widgets.Label(ColRectGap(leftMag, widthMag), trans(leftString));
        }
        ...
        {
            Tooltip(ColRect(0, rectLeftMag + rectWidthMag), _tooltip);
            Label(label);

            return (int)Widgets.HorizontalSlider(PrevColRect(rectLeftMag, rectWidthMag), value, leftValue, rightValue, true, null, leftValue.ToString(), rightValue.ToString());
        }
        ...
        public void GapLine(int height = 12)
        {
            ...
            Widgets.DrawLineHorizontal(tmp.x,tmp.y+tmp.height/2,tmp.width);
        }
