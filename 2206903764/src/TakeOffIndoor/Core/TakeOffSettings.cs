...
namespace TakeOffIndoor
{
    ...
    public class TakeOffSettings : ModSettings
    {
        ...
        public static int nowVerdate = 20221205; //TakeOffWorldComponentも修正
        ...
        public static bool usePreset = false;
        public static List<TOCPreset> presets = new List<TOCPreset>();
        public static int presetUpdateCount = 0;

        public static CustomApparelLayerDictionary customApparelLayer = new CustomApparelLayerDictionary();
        ...
        {
            //Scribe_Values.Look(ref debug.deb, "debug");
            ...
            Scribe_Deep.Look(ref customApparelLayer, "customApparelLayer", null);
            ...
            {
                if (customApparelLayer == null)
                    customApparelLayer = new CustomApparelLayerDictionary();

                if (verdate <= 20221112) //互換処理
                {
                    List<string> neverDefNameList=new List<string>();
                    List<string> invisibleDefNameList = new List<string>();
                    List<string> visibleDefNameList = new List<string>();
                    List<string> forceDefNameList = new List<string>();
                    Dictionary<string, VisibleMode> defNameList;
                    ...
                    foreach(KeyValuePair<string,VisibleMode> kvp in defNameList)
                    {
                        if (customApparelLayer.DefsDicString.ContainsKey(kvp.Key))
                        {
                            customApparelLayer.DefsDicString[kvp.Key].Visibly = kvp.Value;
                        }
                        else
                        {
                            customApparelLayer.DefsDicString[kvp.Key] = new DefsCustomData(kvp.Key, LayerPriority.Null, kvp.Value);
                        }
                    }
                }
            ...
            Scribe_Values.Look(ref usePreset, "usePreset", false);
            Scribe_Collections.Look(ref presets, "presetList", LookMode.Deep, null); // 初期化指定はnullにしないと何故かコンストラクタ無しエラーが出てうまくいかない
