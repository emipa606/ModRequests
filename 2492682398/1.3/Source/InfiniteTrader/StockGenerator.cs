			...
			foreach (var chosenThingDef in DefDatabase<ThingDef>.AllDefs.Where((ThingDef d) => HandlesThingDef(d) && d.tradeability.TraderCanSell()))
			{
				try
                {
					...
					else
                    {
                        ...
                        {
							foreach (Thing item in StockGeneratorUtility.TryMakeForStock(chosenThingDef, 9999999, faction))
							{
						...
						else
                        {
							...
							foreach (Thing item in StockGeneratorUtility.TryMakeForStock(chosenThingDef, chosenThingDef.stackLimit > 1 ? InfiniteTraderMod.settings.bulkAmount : 20, faction))
							{
								generatedThings.Add(item);
							}
						}
