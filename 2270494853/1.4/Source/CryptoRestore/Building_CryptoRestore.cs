...
namespace CryptoRestore
{
    public class Building_CryptoRestore : Building_CryptosleepCasket
    {
        ...
        private static bool HasInjuryHediffs(Pawn pawn)
        {
            ...
            pawn.health.hediffSet.GetHediffs<Hediff_Injury>(ref resultHediffs);

            return resultHediffs.Any();
        }
            ...
            if (pawn != null)
            {
                ...
                foreach (HediffDef chronicInjury in RBank.AgeHediffs)
                {
                    ...
                    if (healedHediffs.Any())
                    {
                        ...
                        {
                            HealthUtility.Cure(healedHediffs.First());
                            ChronicHediffHealTime += RBank.Settings.HealChronicHediffCooldownBase;
                        }
        ...
        private void TryHealBadHediff(Pawn pawn)
        {
            ...
            foreach (var hed in pawn.health.hediffSet.hediffs)
            {
                ...
                if (hed.def.countsAsAddedPartOrImplant) continue;
                if (hed is Hediff_Injury) continue;
                if (hed.def.isBad)
                {
                    HealthUtility.Cure(hed);
                    LameHediffHealTime += RBank.Settings.HealBadHediffCooldownBase;
                    return;
                }
            }
        ...
        private bool TryHealInjuries(Pawn pawn, int ticks)
        {
        ...
        protected virtual void TickPeriod(int ticks = 1)
        {
            ...
            if (hasInjuries)
            {
                hasInjuries = TryHealInjuries(pawn, ticks);
            }
        ...
        public override void EjectContents()
        {
            ...
            if (psycheBroken)
            {
                ...
                else pawn.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.Berserk, null, false, false, null);
            }
        }
        ...
        private void ApplyPersonalNightmare(Pawn pawn)
        {
            ...
            //Log.Error("num150: " + nightmareTicks);
            ...
            //Log.Error("nightmarePercent: " + nightmarePercent);
            ...
            if (pawn.needs.mood != null)
            {
                ...
                //Log.Error("ApplyPersonalNightmareTicks");
