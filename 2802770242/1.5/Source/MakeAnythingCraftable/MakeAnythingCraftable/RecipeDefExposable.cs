...
namespace MakeAnythingCraftable
{
    ...
    public class RecipeDefExposable : RecipeDef, IExposable
    {
        ...
        public bool IsValid()
        {
            if (productString?.defName.NullOrEmpty() is false)
            {
                var def = DefDatabase<ThingDef>.GetNamedSilentFail(productString.defName);
                if (def is null)
                {
                    return false;
                }
        ...
        public RecipeDefExposable(RecipeDef source)
        {
            ...
            jobString = source.jobString;
            workAmount = source.WorkAmountTotal(null);
            allowMixingIngredients = source.allowMixingIngredients;
            ...
            if (!source.ingredients.NullOrEmpty())
            {
                foreach (var ingredient in source.ingredients)
                {
                    ...
                    {
                        foreach (string def in ingredient.filter.categories)
                        {
            ...
            {
                productString = exposable.productString;
                workSpeedStatString = exposable.workSpeedStatString;
                efficiencyStatString = exposable.efficiencyStatString;
                effectWorkingString = exposable.effectWorkingString;
                soundWorkingString = exposable.soundWorkingString;
                workSkillString = exposable.workSkillString;
                unfinishedThingDefString = exposable.unfinishedThingDefString;
                workerType = exposable.workerType;
                ingredientValueGetterType = exposable.ingredientValueGetterType;
                recipeUsersString = exposable.recipeUsersString;
                researchPrerequisitesString = exposable.researchPrerequisitesString;
                skillRequirementsString = exposable.skillRequirementsString;
                ingredientsCount = exposable.ingredientsCount;
                disallowedIngredients = exposable.disallowedIngredients;
            }
        }

        public void ModifyRecipe(RecipeDef recipeDef)
        {
            recipeDef.label = label;
            recipeDef.description = description;
            recipeDef.recipeUsers = recipeUsers;
            recipeDef.researchPrerequisite = null;
            recipeDef.researchPrerequisites = researchPrerequisites;
            recipeDef.effectWorking = effectWorking;
            recipeDef.soundWorking = soundWorking;
            recipeDef.workSpeedStat = workSpeedStat;
            recipeDef.efficiencyStat = efficiencyStat;
            recipeDef.workSkill = workSkill;
            recipeDef.unfinishedThingDef = unfinishedThingDef;
            recipeDef.skillRequirements = skillRequirements;
            recipeDef.products = products;
            recipeDef.adjustedCount = adjustedCount;
            recipeDef.fixedIngredientFilter = fixedIngredientFilter;
            recipeDef.defaultIngredientFilter = defaultIngredientFilter;
            recipeDef.ingredients = ingredients;
            recipeDef.workAmount = workAmount;
            recipeDef.allowMixingIngredients = allowMixingIngredients;
        }
        ...
        public override void ResolveReferences()
        {
            ...
            foreach (string defName in recipeUsersString)
            {
            ...
            if (!researchPrerequisitesString.NullOrEmpty())
            {
                ...
                foreach (string defName in researchPrerequisitesString)
                {
                ...
                foreach (var skillRequirement in skillRequirementsString)
                {
                    if (!skillRequirements.Any(x => x.skill.defName == skillRequirement.skill))
                    {
                        skillRequirements.Add(new SkillRequirement
                        {
                            ...
                            skill = DefDatabase<SkillDef>.GetNamedSilentFail(skillRequirement.skill)
                        });
                    }
                }
            }
            ...
            var product = new ThingDefCountClass(DefDatabase<ThingDef>.GetNamedSilentFail(productString.defName), productString.count);
            if (product != null)
            {
                products.Add(product);
            }
            adjustedCount = product.count;
            ...
            if (ingredientsCount != null)
            {
                ...
                foreach (var ingredientCategory in ingredientsCount)
                {
                    ...
                    {
                        foreach (string item in ingredientCategory.categories)
                        {
                    ...
                    {
                        foreach (string defName in ingredientCategory.thingDefs)
                        {
                            var def = DefDatabase<ThingDef>.GetNamedSilentFail(defName);
                            if (def != null)
                            {
                                ...
                                fixedIngredientFilter.SetAllow(def, allow: true);
                            }
                        }
                    }
        ...
        public override string ToString()
        {
            ...
            {
                text += "\nIngredientsCount: ";
                foreach (var ingredient in ingredientsCount)
                {
