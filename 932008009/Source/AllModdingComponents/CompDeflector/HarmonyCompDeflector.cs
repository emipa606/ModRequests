...
namespace CompDeflector
{
    ...
    internal static class HarmonyCompDeflector
    {
            ...
            float aimAngle)
        {
            ...
            var pawn_EquipmentTracker = wielder?.equipment;
            if (pawn_EquipmentTracker != null)
            {
                foreach (var thingWithComps in pawn_EquipmentTracker.AllEquipmentListForReading)
                {
                    var compDeflector = eq?.GetCompDeflector(); //formerly thingWithComps
            ...
            if (__instance is Pawn pawn)
            {
                ...
                if (pawn_EquipmentTracker != null)
                    foreach (var thingWithComps in pawn_EquipmentTracker.AllEquipmentListForReading)
                    {
                        ...
                        {
                            compDeflector.PostPreApplyDamage(ref dinfo, out var absorbed);
                            if (absorbed)
                            {
