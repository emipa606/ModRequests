...
namespace CompActivatableEffect
{
    ...
    internal static class HarmonyCompActivatableEffect
    {
        ...
        ///     In 1.5, this was moved to PawnRenderUtility and the Pawn section was removed.
        /// </summary>
        public static void DrawEquipmentAimingPostFix(Thing eq, Vector3 drawLoc,
            float aimAngle)
        {
            var compActivatableEffect = eq?.GetCompActivatableEffect();
            ...
            var weaponWielder = (eq.ParentHolder as Pawn_EquipmentTracker)?.pawn;
            var weaponComp = compActivatableEffect.GetOversizedWeapon;
            if (weaponComp != null && weaponWielder != null)
            {
                offset = OffsetFromRotation(weaponComp.Props, weaponWielder.Rotation);
            }
