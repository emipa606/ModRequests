...
namespace JecsTools
{
    ...
    public class BackstoryDef : RimWorld.BackstoryDef
    {
        ...
        public bool CommonalityApproved(Gender g) => Rand.Range(minInclusive: 0, maxInclusive: 100) <
                                                     (g == Gender.Female
        ...
        public override void ResolveReferences()
        {
            ...
            this.forcedTraits = (this.forcedTraits ??= new List<BackstoryTrait>()).Concat(this.forcedTraitsChance
                    .Where(predicate: trait => Rand.Range(minInclusive: 0, maxInclusive: 100) < trait.chance).ToList()
            ...
            this.disallowedTraits = (this.disallowedTraits ??= new List<BackstoryTrait>()).Concat(this
                    .disallowedTraitsChance.Where(predicate: trait => Rand.Range(minInclusive: 0, maxInclusive: 100) < trait.chance)
                    .ToList()
                    .ConvertAll(converter: trait => new BackstoryTrait { def = trait.defName, degree = trait.degree }))
                .ToList();
