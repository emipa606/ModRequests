...
namespace FilterableDesignator
{
	public abstract class Designator_Patch
	{
		...
		private protected static FirstTarget firstTarget = new FirstTarget();
		...
		public class LabelCapReverseDesignating_Patch
		{
			...
			{
				if (__instance.PatchInstance() is Designator_Patch)
				{
		...
		{
			[HarmonyPrefix]
			static bool Prefix(Command __instance, out Texture2D __state)
			{
				__state = null;
			...
			[HarmonyPostfix]
			static void Postfix(Command __instance, Texture2D __state)
			{
				if (__state != null)
				{
