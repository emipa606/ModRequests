...
namespace FilterableDesignator
{
	public static class Utility
	{
		...
		public static void BuildFloatMenuOption<T>(Designator instance, Event ev, List<(T mode, ThingDef thingDef, string text, Texture2D icon)> floatMenuList, Action<T, ThingDef> setMode)
		{
			...
			foreach (var floatMenuRow in floatMenuList)
			{
				...
				Texture2D icon = floatMenuRow.icon ?? iconThingDef?.uiIcon;
