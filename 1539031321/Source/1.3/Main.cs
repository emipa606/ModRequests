...
namespace StatueOfColonist {
    ...
    class Blueprint_Install_SpawnSetup_Patch {
        static void Postfix(Blueprint_Install __instance) {
            Building_StatueOfColonist statue = __instance.MiniToInstallOrBuildingToReinstall.GetInnerIfMinified() as Building_StatueOfColonist;
            if (statue != null) {
                if (!__instance.Map.dynamicDrawManager.DrawThingsForReading.Contains(__instance)) {
                    __instance.Map.dynamicDrawManager.RegisterDrawable(__instance);
                }
            }
        }
    }
    ...
    class Blueprint_Install_DeSpawn_Patch {
        static void Prefix(Blueprint_Install __instance) {
            Building_StatueOfColonist statue = __instance.MiniToInstallOrBuildingToReinstall.GetInnerIfMinified() as Building_StatueOfColonist;
            if (statue != null) {
                if (__instance.Map.dynamicDrawManager.DrawThingsForReading.Contains(__instance)) {
                    __instance.Map.dynamicDrawManager.DeRegisterDrawable(__instance);
                }
            }
        }
    }
    ...
    [HarmonyPatch("Draw")]
    class ThingWithComps_Draw_Patch {
            ...
            if (bp != null) {
                ...
                if (statue != null) {
                    if (!statue.IsValid) {
                        ...
                        Log.Message("ThingWithComps.Draw : ResolveGraphics");
                    }
