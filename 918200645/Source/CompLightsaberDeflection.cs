...
namespace SWSaber
{
    public class CompLightsaberDeflection : CompDeflector.CompDeflector
    {
        ...
        private float CalculatedBlock_ForceModifier()
        {
            ...
            //Accessing a variable from Project Jedi (Star Wars the Force), so let's use a try/catch block...
            try
            {
                var modifier = (int?)AccessTools.Method(forceUser.GetType(), "ForceSkillLevel")
                    .Invoke(forceUser, new object[] { "PJ_LightsaberDefense" });
                ...
                //Increase the random chance of deflection between %15 and 25% for each rank of the lightsaber reflection skill
                for (var i = 0; i < modifier; i++)
                {
            ...
            {
                //Even with no skill, there's a chance...
                return Rand.Range(0.02f, 0.08f);
            }
